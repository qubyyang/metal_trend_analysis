...
- **LLM 分析层深化**：
  - **输出契约化**：请求 `response_format=json_object`（网关不支持时自动回退），
    返回值经 `_validate_analysis` 强制校验 —— 枚举值归一（支持中英文别名）、
    目标价非数字置 null、`key_points` 上限 5 条，异常一律降级并记入
    `schema_warnings`，不再让脏数据流向报告与信号记录。
  - **JSON 提取修复**：`_try_parse_json` 此前无法解析 ```json 代码块包裹的输出
    （`rfind('}')` 会被围栏截断），现已兼容纯 JSON / 代码块 / 前后带说明三种形态。
  - **回测证伪结论注入 prompt**：把「技术信号 p=0.3763 无显著性、滚动 IC 为负」
    写进提示词，明确要求 LLM 不得把技术评分当作方向证据，
    仅依赖技术面时置信度必须标"低"。防止 LLM 为已证伪的信号背书。
  - **因子明细注入**：LLM 现在看到六因子各自的分数与剔除情况，而非只有结论分。
  - **新闻因果分析要求**：区分原因驱动型 / 反应滞后型 / 预期引导型，
    明确"反应滞后型新闻不构成方向证据，把它当依据会形成循环论证"。
  - **LLM 层纳入证伪链路**：`signal_tracker` 新增记录 `llm_confidence` /
    `llm_risk_level` / `llm_parse_mode`；`--backtest` 现对 LLM 路同样输出
    买入持有基准对比（此前仅技术路有），并新增 **LLM 置信度分层胜率**
    检验"高置信度是否真的更准"。

### 修复

- **`_build_prompt` 死代码**：一个 215 行、含 JSON 契约与新闻因果分析要求的
  prompt 构造器从未被任何代码调用，实际生效的是功能简化的
  `_build_analysis_prompt`。两者已合并为一套，死代码删除。
- **分层统计静默丢样本**：`evaluate()` 的 `details` 仅保留最后 20 条用于展示，
  若分层统计读它会静默丢弃绝大部分样本。抽出 `_collect_evaluated` 返回全量，
  并加回归测试锁定。
- **解析失败时输出 `'N/A'` 方向**：会污染 `signal_tracker` 的方向归一，
  现降级为合法保守值（中性 / 中 / 低置信度）。

- **因子调优验证** `scripts/tune_verify.py` + `docs/因子调优验证报告.md`：
  对六因子做 Spearman IC 检验、IS/OOS 权重网格搜索、滚动 IC 稳定性检验。
  结论为**负面**：黄金全因子 |IC| < 0.09，白银多数为负；IS 最优权重在 OOS
  全面翻负；滚动 1 年 IC 均值 XAU -0.269 / XAG -0.246，9 个窗口中 7-8 个为负。
  据此**不调整 `DEFAULT_WEIGHTS`、不延长持有期、不做白银反转**。
  信号引擎定位修订为纪律框架而非盈利信号。

- **历史信号回填** `SignalBackfiller`（`--backfill`）：用历史 K 线逐日重放生成
  信号记录。此前靠每日采集攒到统计显著（≥30 条已到期）需要数月，
  回填一次可得数百条样本。
  - 前视偏差三道防线：逐日截断计算、指标因果性由测试断言
    （「截断重算 == 全量切片」，引入非因果指标会立刻失败）、
    评估端只取 `index > entry_time` 的价格。另有测试篡改未来数据，
    断言历史信号评分完全不变。
  - **采样间隔默认等于持有期**而非逐日。持有期 5 日却每日采样会使相邻样本
    评估窗口重叠 80%，把二项检验的标准误低估约 sqrt(5) 倍。
  - 记录带 `source="backfill"` 标记，可与实盘信号分开统计；重复回填幂等。
- **回测统计维度扩展**：
  - **买入持有基准对比**：胜率脱离基准无意义——牛市里只喊多也有 55% 胜率。
    报告并列展示策略择时与同期买入持有的期望收益，跑输时明确警示。
  - **二项显著性检验**：样本 ≥30 时给出 z 值与 p 值，否则明确标注样本不足，
    而非输出误导性的 p 值。
  - **按信号强度分层胜率**：强/中/弱三档分别统计，检验因子评分是否真有区分度。
  - **持有期衰减曲线**：1/3/5/10/20 日各自的胜率与盈亏比。
  - **最大回撤**：信号序列按时间排序等权累计（非真实账户回撤）。
  - `evaluate` 新增 `horizon_days` 与 `source` 参数。
- **每日采集自动化**：`scripts/daily_signal_cron.sh`（交易日采集，周一额外回测）
  与 `scripts/com.metaltrend.daily.plist`（macOS launchd，休眠错过会补跑）。
- 新增 `backfill` 配置段。

### 回测结论（首次真实评估）

用回填的 242 条 XAUUSD 非重叠样本（2021-05 ~ 2026-08）评估当前技术信号：
胜率 53.26%，但 **z=+0.88 / p=0.3763，与随机无显著差异**；
期望收益 +0.111%/5 日，**低于买入持有基准的 +0.220%**；
强信号档胜率 53.2% 与中等档 53.5% 几无差异，评分缺乏区分度。
白银更差（强信号档 46.8% 反低于中等档 55.6%）。

结论如实保留：当前信号尚未证明具备 alpha，不应据此实盘。
下一步应检查因子权重与持有期设置，而非继续增加因子。

- **因子化信号引擎** `SignalEngine`：把趋势判断从「看涨/看跌信号计数投票」
  改为六因子加权连续评分（-100~+100）。
  - 因子：均线排列 0.25 / MACD 0.20 / RSI 0.15 / 布林带 0.15 /
    多周期共振 0.15 / 成交量 0.10，权重与阈值均可通过 `signal_engine` 配置段调整，
    权重设为 0 即关闭该因子。
  - **无效因子重归一化**：数据缺失的因子被剔除后，只用有效因子的权重作为
    加权平均的分母。此前把它们当作中性票参与投票，等价于用噪声稀释有效信号。
    典型场景是新浪现货金银 `volume` 恒为 0，实测有效权重 0.90 而非 1.00。
  - **ATR 波动率闸门**：`ATR/收盘价` 超过阈值（默认 3%）时线性衰减评分
    （下限 50%），只降置信度、不改方向。
  - RSI 按摆动指标的反转语义计分（超买为负），与趋势类因子刻意相反。
  - 评分、方向、置信度写入 `signal_score` / `signal_direction` /
    `signal_confidence` 字段，并落入信号回测记录，供后续按信号强度分层统计胜率。
  - 报告新增「综合信号评分」段落，输出逐因子的强度、权重与说明，结论可追溯。
  - `calculate_all_indicators` 补充输出 `ATR` 列（此前已实现但未接入主流程）。
