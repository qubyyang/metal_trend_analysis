...
- **🤖 AI驱动分析**: 集成GPT-4等大语言模型，输出经 schema 校验的结构化研判；提示词内置回测证伪结论，约束模型不得为无统计显著性的技术信号背书
...
技术信号已被证伪，因此 LLM 层的设计目标不是"让结论更好听"，而是**让它可验证**。

### 提示词里写着技术信号不可信

回测结论被直接注入提示词，模型必须遵守：

- 不得把技术评分本身当作看涨/看跌的独立依据，它没有被证实的预测力
- 技术指标只能描述"当前市场状态"，不能作为"未来方向的证据"
- 若结论主要依赖技术指标，置信度**必须**标为"低"
- 缺乏有信息量的证据时，应如实给出"中性 + 低置信度"，不要编造理由

同时模型能看到六因子各自的分数与剔除情况，而不是只有一个结论分——
让它自己判断证据强度，而非复读结论。

### 新闻要分因果方向

提示词要求把每条新闻归类为**原因驱动型**（美联储决议、通胀数据、地缘冲突）、
**反应滞后型**（"金价创新高因避险情绪升温"）或**预期引导型**（待公布数据）。

关键约束：反应滞后型新闻不构成方向证据——它只是价格的镜像，
把它当依据会形成循环论证。若全部新闻都是滞后型，模型须声明"新闻无独立信息量"并下调置信度。

### 输出契约化，不盲信模型

请求 `response_format=json_object`（网关不支持时自动回退），返回值强制校验：

| 检查 | 处理 |
|------|------|
| 枚举值非法（如 trend="暴涨"） | 归一到白名单，中英文别名均支持，否则降级为保守值 |
| 目标价非数字 | 置 null，绝不猜测 |
| 高置信度但无支撑要点 | **强制下调为中**，并记录原因 |
| 未返回合法 JSON | 回退正则抽取，置信度一律标"低" |
| 解析彻底失败 | 返回合法保守值，不输出 `'N/A'` 污染下游 |

所有降级都写入 `schema_warnings` 字段，可追溯。

### LLM 同样要接受证伪

`--backtest` 现在对 LLM 路输出与技术路**同等**的检验：胜率、显著性、
买入持有基准对比，外加**置信度分层胜率**——验证模型说"高置信度"时是否真的更准。

若高置信档胜率不高于低置信档，报告会直接判定"置信度缺乏区分度，应视为措辞而非信息"。
LLM 不因为"是 AI 给的"就豁免于证据标准。
